...
export default function FraudSimulation({ onSimulate, activeScenario, isStreaming, onToggleStreaming, transactions }: FraudSimulationProps) {
   ...
   return (
      <div className="rounded-[2rem] p-8 border border-slate-200 bg-white mb-10 overflow-hidden relative group shadow-sm">
         ...
         <div className="relative z-10 flex flex-col xl:flex-row items-center gap-10">
            ...
            {/* Controls Section */}
            <div className="flex-1 flex flex-col md:flex-row items-start md:items-center justify-between gap-8 w-full border-r border-slate-200 pr-0 xl:pr-10">
               <div className="space-y-4 w-full md:w-auto">
                  <div className="flex items-center gap-6">
                     <div>
                        <h2 className="text-2xl font-bold font-space uppercase tracking-tight text-slate-900 flex items-center gap-3">
                           <Activity size={22} className="text-primary animate-pulse" />
                           Forensic SIMULATOR
                        </h2>
                        <p className="text-[10px] text-slate-500 mt-1 font-bold uppercase tracking-[0.2em] opacity-60">
            ...
            <div className="w-full xl:w-[45%] grid grid-cols-2 lg:grid-cols-3 gap-6">
               <div className="space-y-1.5 p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  ...
                  <p className="text-lg font-bold font-space text-slate-900">${stats.total.toLocaleString()}</p>
               </div>
               ...
               <div className="hidden lg:block space-y-1.5 p-4 rounded-2xl bg-slate-50 border border-slate-200 overflow-hidden relative">
